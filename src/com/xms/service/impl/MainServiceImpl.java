...
public class MainServiceImpl implements MainService{
	...
	public List<Course> findNewCourse() {
		...
		if(newCourses.size() <= 10) {
			...
			for(int i=0;i<10;i++) {
				listCourses.add(newCourses.remove(
	...
	public List<Course> findCourse(int contentId, int size) {
		...
		List<Course> courses = 
			mainMapper.findCourseByContentId(contentId);
		...
		if (courses.size() <= size) {
			...
			for(int i=0;i<size;i++) {
				listCourses.add(courses.remove(
					random.nextInt(courses.size())));
			}
			return listCourses;
		}
	}
